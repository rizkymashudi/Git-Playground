...
export function gitBranch(repo: Repository, args: string[]): GitResult {
  ...
  if (args[0] === '-d' || args[0] === '-D') {
    ...
    if (name === r.HEAD) return { repo: r, output: `error: Cannot delete branch '${name}' checked out`, success: false };
    if (!r.branches[name]) return { repo: r, output: `error: branch '${name}' not found`, success: false };
    delete r.branches[name];
    return { repo: r, output: `Deleted branch ${name}`, success: true };
  }

  const name = args[0];
  if (r.branches[name]) return { repo: r, output: `fatal: branch '${name}' already exists`, success: false };
